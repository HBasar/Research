...
# Enhanced VGG16 with improved architecture
class OptimizedVGG16(nn.Module):
    def __init__(self, num_classes=4, pretrained=True, dropout_rate=0.5):
        ...
        # Fine-tune last conv blocks only
        for i, param in enumerate(self.vgg16.features.parameters()):
            if i < 24:  # Freeze early layers
                param.requires_grad = False
        
        num_features = self.vgg16.classifier[0].in_features
        
        # Optimized classifier with residual-like connections
...
def train_model(model, train_loader, valid_loader, num_epochs=100, learning_rate=0.0001):
    ...
    if os.path.exists('best_vgg16_model.pth'):
...
def main():
    ...
    try:
        print("="*80)
        print("OPTIMIZED VGG16 DISEASE CLASSIFICATION - 100 EPOCHS")
        print("="*80 + "\n")
        ...
        print("="*80)
        model = OptimizedVGG16(num_classes=num_classes, pretrained=True, dropout_rate=0.5)
        ...
        if len(y_pred) > 0:
            ...
            # Save final model
            torch.save(trained_model.state_dict(), 'optimized_vgg16_final.pth')
            print("\nFinal model saved as 'optimized_vgg16_final.pth'")
